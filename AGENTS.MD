...
最近基线：Router 99、共享会话 29、authz 网关 882（含实例级 API Key x-api-key 免登录、IP/CIDR 白名单、
 以及 guest 角色与 `/_authz/guest` 探针套件：默认角色 guest、探针 + 只回显自身的
 `GET /api/session` 之外的控制面/管理页/文件浏览全拒、回显转义与请求头完整明文回显、角色升降即时生效、
密钥指纹 `token_prefix` 与 `POST /api/api-keys/:id/rotate` 轮换（旧明文当场失效）。
viewer 角色已退役（迁移 18 把存量 viewer 就地改写为 guest；现在提交 `viewer` 直接 422，
不再有映射兼容）。
...
## 测试实例（241.t）
页面测试使用 playwright-cli（无认证写法见下）
`x-api-key` 是页面级免认证的唯一正道：playwright-cli 没有全局 `--header` 参数，
用 `run-code` 调 Playwright API 给整个页面上下文注入请求头，之后所有导航和
子资源请求都自动带 Key，无需登录取 Cookie，也无需任何本地注入代理：
...
注意：Key 只认请求头（无 query 参数回退），来源受 AUTHZ_API_KEY_ALLOWED_IPS 限制；
自签 HTTPS（6443）用 `chromium.launch({ ignoreHTTPSErrors })` 不适用于已开会话，
直接测 http 端口 6080 即可。
`241.t`（10.252.25.241）是本项目专用测试/回归实例，生产实例跑在本机（writer）。

 **所有回归测试一律在 241.t 上运行；除非用户明确要求，不要向本机生产实例部署新代码或镜像**
 （rsync 与 docker compose 只针对 241.t 的 /data/app/authz-test/，不动本机 /data/app/）。
