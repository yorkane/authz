-- Vendored verbatim from Kong/lua-resty-aws 1.7.2 (commit a73c39c, Apache-2.0,
-- see ../LICENSE.lua-resty-aws). The only local change is the "pl_string.strip"
-- call replaced with plain Lua string trimming, so the signer does not drag
-- Penlight into the image. Nothing else in this file was touched.
...
-- Header values are whitespace-trimmed before they enter the canonical request
-- (RFC 4757 § 5: sequential/internal whitespace collapse aside, leading and
-- trailing spaces are not part of the signed value). Replaces
-- pl.stringx.strip() from the upstream dependency chain.
...
local function prepare_awsv4_request(config, request_data)
  ...
  -- LOCAL PATCH (authz): honour a caller-supplied X-Amz-Content-Sha256. Streaming
  -- uploads sign the literal "UNSIGNED-PAYLOAD" instead of the body digest (the
  -- private endpoint accepts it) and that literal must be what enters the canonical
  -- request too. Without this the server answers SignatureDoesNotMatch.
  local supplied_payload_hash = req_headers["X-Amz-Content-Sha256"]
      or req_headers["x-amz-content-sha256"]
  if S3 and supplied_payload_hash then
    hashed_payload = tostring(supplied_payload_hash)
  elseif S3 then
  ...
  local canonical_headers, signed_headers do
    ...
    for name, value in pairs(headers) do
      if value then -- ignore headers with 'false', they are used to override defaults
        ...
        canonical_headers[name_lower] = strip(tostring(value))
      end
    end
