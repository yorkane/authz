  ...
  const messages = {
    'zh-CN': {
      ...
      authorization: {
        ...
        rwConditions: '生效条件', rwConditionsHint: '整条改写只在条件命中时生效（对齐 APISIX route vars）。URI 匹配含查询串的请求路径；条件留空则对所有响应生效。',
        rwCondLogic: '条件组合', rwCondLogicAll: '全部满足（AND）', rwCondLogicAny: '任一满足（OR）',
        rwCondField: '匹配对象', rwCondName: 'Header 名称', rwCondOp: '匹配方式', rwCondValue: '值',
        rwCondValueHint: '正则留空默认按正则匹配；值可写成 /re/ 形式，两端斜杠会被去掉。',
        rwCondNegate: '取反该条件', rwAddCondition: '添加条件', rwCondNone: '尚未添加条件：改写对所有响应生效',
        rwCondFieldUri: '请求 URI', rwCondFieldRequestHeader: '请求头', rwCondFieldResponseHeader: '响应头',
        rwCondFieldContentType: 'Content-Type', rwCondFieldStatus: '响应状态码',
        rwCondOpEquals: '等于', rwCondOpContains: '包含', rwCondOpRegex: '正则', rwCondOpExists: '存在', rwCondOpMissing: '不存在',
        rwCondHeaderHint: 'Header 名称，不区分大小写', rwCondUriHint: '对完整请求路径（含查询串）做匹配',
        ...
        rrHeaders: 'Request headers', rrHeadersHint: 'Three operations per header, APISIX-style: set replaces the value, append adds to it (managed headers merge with "; " / "," separators, plain headers gain an extra line), remove drops it. Host, Cookie, Origin, X-Forwarded-* and X-Authz-User/Source/Identity are all rewritable and win over gateway defaults. Framing, hop-by-hop and gateway credential headers cannot be rewritten.',
        rrOpSet: 'Set', rrOpAppend: 'Append', rrOpRemove: 'Remove',
        rrOpHint: 'Set replaces the value; append merges into the current value; remove deletes the header. Set and append are mutually exclusive per header.',
        rrAppendPlaceholder: 'value to append (e.g. session=abc or 1.2.3.4)',
        ...
        rrHeaders: '请求头改写', rrHeadersHint: '参考 APISIX，每个头支持三种操作：替换（覆盖当前值）、追加（托管头按 "; "/"," 分隔并入现值，普通头多出一行）、删除。Host、Cookie、Origin、X-Forwarded-*、X-Authz-User/Source/Identity 等网关托管头均可改写，改写值优先于网关默认值。分帧、hop-by-hop 与网关凭据头（X-Authz-Key/X-API-Key/X-Role-Key）不可改写。',
        rrOpSet: '替换', rrOpAppend: '追加', rrOpRemove: '删除',
        rrOpHint: '替换覆盖当前值；追加把值并入当前值；删除移除该头。同一个头不能同时替换和追加。',
        rrAppendPlaceholder: '要追加的值（如 session=abc 或 1.2.3.4）',
    ...
    'en-US': {
      ...
      authorization: {
        ...
        rwConditions: 'Match conditions', rwConditionsHint: 'The rewrite only applies when the conditions match (aligned with APISIX route vars). URI matches the request path including the query string; with no conditions the rewrite applies to every response.',
        rwCondLogic: 'Condition logic', rwCondLogicAll: 'All must match (AND)', rwCondLogicAny: 'Any may match (OR)',
        rwCondField: 'Match target', rwCondName: 'Header name', rwCondOp: 'Operator', rwCondValue: 'Value',
        rwCondValueHint: 'Blank operator defaults to regex; wrap the value as /re/ and the slashes are stripped.',
        rwCondNegate: 'Negate this condition', rwAddCondition: 'Add condition', rwCondNone: 'No conditions yet: the rewrite applies to every response',
        rwCondFieldUri: 'Request URI', rwCondFieldRequestHeader: 'Request header', rwCondFieldResponseHeader: 'Response header',
        rwCondFieldContentType: 'Content-Type', rwCondFieldStatus: 'Response status',
        rwCondOpEquals: 'Equals', rwCondOpContains: 'Contains', rwCondOpRegex: 'Regex', rwCondOpExists: 'Exists', rwCondOpMissing: 'Missing',
        rwCondHeaderHint: 'Header name, case-insensitive', rwCondUriHint: 'Matched against the full request path including the query string',
