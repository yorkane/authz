  ...
  const messages = {
    'zh-CN': {
      ...
      authorization: {
        ...
        bindingTitle: '新增域名绑定', editBindingTitle: '编辑域名绑定', bindingCopy: '填写最后一级前缀与目标地址；代理细节按需展开。', domainPlaceholder: 'name1', domainHint: '只填最后一级前缀（如 code）；网关按 <前缀>-<节点>.<当前访问域名> 自动拼出完整域名，同一套绑定可在多个入口域名下使用。也可直接填完整域名', targetIpPlaceholder: '127.0.0.1', targetIpHint: '默认代理本机；也可填写其他机器的 IPv4 或 IPv6 地址', menuNameHint: '填写后直接作为左侧菜单名称', websocketEnabled: 'WebSocket 自动代理', websocketHint: '所有目标默认支持升级请求', enabledNow: '立即启用', createBinding: '创建绑定', saveBinding: '保存绑定', editBinding: '编辑绑定',
