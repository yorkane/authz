  ...
  const messages = {
    'zh-CN': {
      ...
      users: {
        ...
        roleHint: '从 admin、staff、user、guest 中选择一个或多个角色', createAction: '创建用户', rolesTitle: '设置角色', rolesHint: '角色用于统一控制管理和应用访问权限',
        ...
        required: '此项不能为空', usernameRule: '使用 2-32 位小写字母、数字、_ 或 -', passwordRule: '密码至少 6 位', roleRule: '请从 admin、staff、user、guest 中至少选择一个角色',
        ...
        apiTitle: 'API 管理', apiNote: '机器身份密钥：默认 guest 角色，仅可访问 /_authz/guest 诊断页；请求头 x-api-key 提交', apiCreate: '新建 API', apiName: '名称', apiNameRule: '使用 2-64 位字母、数字、点、下划线或连字符', apiRoleHint: 'guest 为默认最小权限；api/admin 等角色按需授予', apiCreateTitle: '新建 API', apiCreateCopy: '保存后仅展示一次密钥明文，请立即复制。', apiCreateAction: '生成密钥', apiTokenTitle: 'API 密钥已生成', apiTokenCopy: '这是该密钥唯一一次明文展示，关闭后无法再次查看。', apiCopy: '复制', apiCopied: '密钥已复制到剪贴板', apiCopyFailed: '复制失败，请手动选择复制', apiRoleTitle: '修改角色', apiRoleUpdated: 'API 角色已更新', apiEnabledDone: 'API 已启用', apiDisabledDone: 'API 已停用', apiDeleteTitle: '删除 API', apiDeleted: 'API 已删除', apiNoData: '暂无 API 密钥', apiPrefix: '密钥指纹', apiRotateTitle: '轮换密钥', apiRotateAction: '立即轮换', apiRotateConfirm: '旧密钥立即失效，使用它的调用会马上收到 401；新密钥明文只展示这一次。', apiRotated: '密钥已轮换，请及时更新调用方', apiRotateTokenTitle: 'API 密钥已轮换'
      },
      authorization: {
        ...
        bindingTitle: '新增域名绑定', editBindingTitle: '编辑域名绑定', bindingCopy: '填写最后一级前缀与目标地址；代理细节按需展开。', domainPlaceholder: 'name1', invalidDomainPrefix: '只能输入最后一级域名前缀（字母、数字和中划线）', targetIpPlaceholder: '127.0.0.1', targetIpHint: '默认代理本机；也可填写其他机器的 IPv4 或 IPv6 地址', menuNameHint: '填写后直接作为左侧菜单名称', websocketEnabled: 'WebSocket 自动代理', websocketHint: '所有目标默认支持升级请求', enabledNow: '立即启用', createBinding: '创建绑定', saveBinding: '保存绑定', editBinding: '编辑绑定',
        ...
        subjectHint: '选择角色或具体用户', userHint: '选择需要分配角色的用户', roleHint: '策略主体支持 admin、staff、user、guest 或 api', objectHint: '按菜单名、域名和目标地址选择；也可输入 /<端口><路径>', objectAddressRule: '请选择绑定对象或输入 /<端口><路径>', objectPathRule: '路径必须以 / 开头，且不能包含空格或逗号', httpActionHint: '可选择多个标准 HTTP 方法；* 表示全部方法', allObjects: '全部地址 · /*', allObjectsHint: '匹配所有绑定、端口和路径', directPort: '直接端口', sharedBindings: '个绑定共享', unboundObject: '未找到对应绑定', invalidObject: '无效对象', allActions: '全部方法 · *', allow: '允许', deny: '拒绝', assigned: '已分配',
        deleteBinding: '删除绑定', deletePolicy: '删除策略',
        responseRewrite: '改写响应', responseRewriteOn: '已配置改写', responseRewriteNone: '未配置改写',
        rwTitle: '改写响应', rwCopy: '改写该绑定返回给浏览器的上游响应：状态码、响应头与正文（语义参考 APISIX response-rewrite）。',
        rwSave: '保存改写', rwNoHeaders: '尚未改写响应头', rwNoFilters: '尚未添加过滤规则',
        rwBodyPlaceholder: '例如 {"ok": true} 或自定义 HTML',
        rwEnabled: '启用改写', rwEnabledHint: '关闭时保留配置但不生效，用于临时停用。',
        rwStatus: '响应状态码', rwStatusHint: '留空表示保持上游状态码；填错会让前端误判请求结果。',
        rwHeaders: '响应头改写', rwHeadersHint: '覆盖上游响应头；勾选删除可移除该头。Set-Cookie、Content-Length、CSP 等网关与安全头不可改写。',
        rwHeaderName: 'Header 名称', rwHeaderValue: '值', rwHeaderDelete: '删除该响应头', rwAddHeader: '添加响应头',
        rwBodyMode: '正文处理', rwBodyNone: '不改写', rwBodyReplace: '整体替换', rwBodyFilter: '内容过滤',
        rwBodyModeHint: '替换会把上游正文整体换成下面填写的内容；过滤只按规则替换匹配的片段。',
        rwBody: '替换正文', rwBodyHint: '支持文本或 JSON；JSON 会自动写回 application/json。',
        rwBodyBase64: 'Base64（二进制内容）', rwBodyBase64Hint: '勾选后上面的内容按 Base64 解码后再返回。',
        rwContentType: 'Content-Type', rwContentTypeHint: '替换正文时写回的类型，留空保持上游类型。',
        rwFilters: '正文过滤规则', rwFiltersHint: '按顺序执行。匹配内容以 ~ 开头（或勾选正则）时按 PCRE 处理，替换支持 $1 捕获组。',
        rwFilterSource: '匹配内容', rwFilterTarget: '替换为', rwFilterRegex: '正则', rwAddFilter: '添加规则',
        rwViewForm: '表单', rwViewJson: 'JSON', rwJson: 'JSON 配置', rwJsonHint: '高级用法：直接编辑完整配置；保存时由服务端统一校验。',
        rwJsonInvalid: 'JSON 格式不正确', rwJsonArrayWarn: '顶层必须是 JSON 对象',
        rwScope: '生效范围', rwScopeHint: '仅上游返回 200 的 GET 响应生效；压缩、分片（Range）、二进制与超过 1MB 的响应会自动跳过。',
        rwSaved: '响应改写已保存', rwCleared: '响应改写已清除', rwClear: '清除改写', rwClearConfirm: '确认清除该绑定的全部响应改写配置？',
        rwUnsafeHint: '改写响应会把网关置于内容与浏览器之间：请只使用可信内容，避免改写安全头或注入脚本。',
        requestRewrite: 'Request rewrite', requestRewriteOn: 'Request rewrite configured',
        rrTitle: 'Request rewrite', rrCopy: 'Rewrite the request headers and text body this binding sends upstream (formerly header overrides). Request status cannot be rewritten.',
        rrSave: 'Save rewrite', rrNoHeaders: 'No request headers yet',
        rrEnabled: 'Enable rewrite', rrEnabledHint: 'When off, the configuration is kept but not applied.',
        rrHeaders: 'Request headers', rrHeadersHint: 'Override request headers sent upstream; tick remove to delete one. Host, Cookie, Origin, X-Forwarded-* and X-Authz-User/Source/Identity can be overridden too (they win over the gateway defaults). Framing, hop-by-hop and gateway credential headers cannot be rewritten.',
        rrHeaderName: 'Header name', rrHeaderValue: 'Value', rrHeaderDelete: 'Remove this header', rrAddHeader: 'Add header',
        rrViewForm: 'Form', rrViewJson: 'JSON', rrJson: 'JSON config', rrJsonHint: 'Advanced: edit the full configuration directly; the server validates it on save.',
        rrJsonInvalid: 'Invalid JSON', rrJsonArrayWarn: 'Top level must be a JSON object',
        rrBodyModeHint: 'Replace swaps the whole request body before forwarding; filter only rewrites matched fragments. Only textual requests with a known Content-Length are rewritten (chunked, binary, GET/HEAD pass through).',
        rrContentType: 'Upstream Content-Type', rrContentTypeHint: 'Written back only when replacing the body; a JSON body defaults to application/json.',
        rrBody: 'Replacement body', rrBodyHint: 'Text or JSON; the Content-Length is recalculated automatically.',
        rrBodyPlaceholder: 'e.g. {"ok": true} or a form body',
        rrFiltersHint: 'Applied in order over the request body. A source starting with ~ (or the regex switch) is treated as PCRE; the replacement supports $1 groups.',
        rrScopeHint: 'Only affects requests sent to this binding upstream; unlisted client headers still pass through. Body rewrite needs a textual Content-Type and a known Content-Length, otherwise the body is forwarded unchanged.',
        rrSaved: 'Request rewrite saved', rrCleared: 'Request rewrite cleared', rrClear: 'Clear rewrite', rrClearConfirm: 'Clear all request rewrite configuration for this binding?',
        rrUnsafeHint: 'Rewriting request headers or body may affect upstream auth and routing: only put trusted values in sensitive headers like Authorization, and keep body filters narrow to avoid corrupting uploads.',
        requestRewrite: '改写请求', requestRewriteOn: '已配置请求改写',
        rrTitle: '改写请求', rrCopy: '改写该绑定发往上游的请求头与文本正文（原「Header 覆盖」）；请求状态码不可改写。',
        rrSave: '保存改写', rrNoHeaders: '尚未改写请求头',
        rrEnabled: '启用改写', rrEnabledHint: '关闭时保留配置但不生效，用于临时停用。',
        rrHeaders: '请求头改写', rrHeadersHint: '覆盖发往上游的请求头；勾选删除可移除该头。Host、Cookie、Origin、X-Forwarded-*、X-Authz-User/Source/Identity 等网关托管头也可改写，改写值优先于网关默认值。分帧、hop-by-hop 与网关凭据头（X-Authz-Key/X-API-Key/X-Role-Key）不可改写。',
        rrHeaderName: 'Header 名称', rrHeaderValue: '值', rrHeaderDelete: '删除该请求头', rrAddHeader: '添加请求头',
        rrViewForm: '表单', rrViewJson: 'JSON', rrJson: 'JSON 配置', rrJsonHint: '高级用法：直接编辑完整配置；保存时由服务端统一校验。',
        rrJsonInvalid: 'JSON 格式不正确', rrJsonArrayWarn: '顶层必须是 JSON 对象',
        rrBodyModeHint: '替换会在转发前把整个请求正文换成填写内容；过滤只替换匹配片段。仅对文本类、Content-Length 明确的请求生效（分块、二进制、GET/HEAD 原样透传）。',
        rrContentType: '上游 Content-Type', rrContentTypeHint: '仅在整体替换正文时写回；JSON 正文默认 application/json。',
        rrBody: '替换正文', rrBodyHint: '支持文本或 JSON；Content-Length 会自动重算。',
        rrBodyPlaceholder: '例如 {"ok": true} 或表单正文',
        rrFiltersHint: '按顺序对请求正文执行。匹配内容以 ~ 开头（或勾选正则）时按 PCRE 处理，替换支持 $1 捕获组。',
        rrScopeHint: '仅影响发往该绑定上游的请求；未列出的客户端头仍按原逻辑透传。正文改写要求文本类 Content-Type 且 Content-Length 明确，否则正文原样转发。',
        rrSaved: '请求改写已保存', rrCleared: '请求改写已清除', rrClear: '清除改写', rrClearConfirm: '确认清除该绑定的全部请求改写配置？',
        rrUnsafeHint: '改写请求头或正文可能影响上游鉴权与路由：Authorization 等敏感头请只配置可信值；正文过滤规则尽量收窄，避免破坏上传内容。',
      ...
      menuEditor: {
        ...
        dynamicGroupHint: '“本地服务”由端口探测自动生成，不在此编辑。',
        domainsGroupHint: '“域名服务”分组自动汇集已绑定域名的应用（含其直连端口入口）；改名会同步到域名绑定的菜单名，可隐藏但不可删除。',
        builtinUndeletable: '内置菜单不能删除',
        labelPlaceholder: '例如 监控面板', urlPlaceholder: '例如 /_authz/apps/users.html 或 https://example.com',
        iconPlaceholder: 'MDI 图标名，例如 mdi-monitor', iconHint: '留空使用默认图标', iconSearchPlaceholder: '搜索图标（关键字，例如 chart、server、cloud…）', iconNoMatch: '没有匹配的图标，换个关键字试试',
    ...
    'en-US': {
      ...
      users: {
        ...
        roleHint: 'Select one or more roles from admin, staff, user, and guest', createAction: 'Create user', rolesTitle: 'Set roles', rolesHint: 'Roles consistently control administrative and application access',
        ...
        required: 'This field is required', usernameRule: 'Use 2-32 lowercase letters, numbers, _ or -', passwordRule: 'Password must be at least 6 characters', roleRule: 'Select at least one of admin, staff, user, or guest',
        ...
        apiTitle: 'API management', apiNote: 'Machine credentials: new keys default to the guest role and can only open /_authz/guest; present them via the x-api-key header', apiCreate: 'New API', apiName: 'Name', apiNameRule: 'Use 2-64 letters, numbers, dots, underscores or hyphens', apiRoleHint: 'guest is the least-privilege default; grant api/admin and others as needed', apiCreateTitle: 'New API', apiCreateCopy: 'The secret is shown once after creation; copy it right away.', apiCreateAction: 'Generate key', apiTokenTitle: 'API key created', apiTokenCopy: 'This is the only time the secret is displayed in plain text. It cannot be shown again.', apiCopy: 'Copy', apiCopied: 'Key copied to clipboard', apiCopyFailed: 'Copy failed; select and copy manually', apiRoleTitle: 'Change role', apiRoleUpdated: 'API role updated', apiEnabledDone: 'API enabled', apiDisabledDone: 'API disabled', apiDeleteTitle: 'Delete API', apiDeleted: 'API deleted', apiNoData: 'No API keys yet', apiPrefix: 'Key fingerprint', apiRotateTitle: 'Rotate key', apiRotateAction: 'Rotate now', apiRotateConfirm: 'The old secret stops working immediately and its callers start failing with 401; the new secret is shown only this once.', apiRotated: 'Key rotated; update your callers', apiRotateTokenTitle: 'API key rotated'
      },
      authorization: {
        ...
        subjectHint: 'Select a role or a specific user', userHint: 'Select the user receiving the role', roleHint: 'Policy subjects support admin, staff, user, guest, or api', objectHint: 'Select by menu, domain, and target; or enter /<port><path>', objectAddressRule: 'Select a binding or enter /<port><path>', objectPathRule: 'Path must start with / and contain no spaces or commas', httpActionHint: 'Select multiple standard HTTP methods; * matches every method', allObjects: 'All objects · /*', allObjectsHint: 'Matches every binding, port, and path', directPort: 'Direct port', sharedBindings: 'shared bindings', unboundObject: 'No matching binding', invalidObject: 'Invalid object', allActions: 'All methods · *', allow: 'Allow', deny: 'Deny', assigned: 'Assigned',
        deleteBinding: 'Delete binding', deletePolicy: 'Delete policy',
        responseRewrite: 'Response rewrite', responseRewriteOn: 'Rewrite configured', responseRewriteNone: 'No rewrite',
        rwTitle: 'Response rewrite', rwCopy: 'Rewrite the upstream response this binding returns to browsers: status, headers and body (semantics follow APISIX response-rewrite).',
        rwSave: 'Save rewrite', rwNoHeaders: 'No response headers yet', rwNoFilters: 'No filter rules yet',
        rwBodyPlaceholder: 'For example {"ok": true} or custom HTML',
        rwEnabled: 'Enable rewrite', rwEnabledHint: 'When off, the configuration is kept but not applied.',
        rwStatus: 'Response status', rwStatusHint: 'Leave blank to keep the upstream status; a wrong code makes clients treat the request as failed.',
        rwHeaders: 'Response headers', rwHeadersHint: 'Override upstream response headers; tick remove to delete one. Set-Cookie, Content-Length, CSP and other gateway/security headers cannot be rewritten.',
        rwHeaderName: 'Header name', rwHeaderValue: 'Value', rwHeaderDelete: 'Remove this header', rwAddHeader: 'Add header',
        rwBodyMode: 'Body handling', rwBodyNone: 'Keep body', rwBodyReplace: 'Replace body', rwBodyFilter: 'Filter body',
        rwBodyModeHint: 'Replace swaps the whole upstream body; filter only rewrites the matched fragments.',
        rwBody: 'Replacement body', rwBodyHint: 'Text or JSON; JSON is sent back with application/json.',
        rwBodyBase64: 'Base64 (binary)', rwBodyBase64Hint: 'Decode the value above from Base64 before sending it.',
        rwContentType: 'Content-Type', rwContentTypeHint: 'Written back when the body is replaced; blank keeps the upstream type.',
        rwFilters: 'Body filter rules', rwFiltersHint: 'Applied in order. A source starting with ~ (or the regex switch) is treated as PCRE; the replacement supports $1 groups.',
        rwFilterSource: 'Match', rwFilterTarget: 'Replace with', rwFilterRegex: 'Regex', rwAddFilter: 'Add rule',
        rwViewForm: 'Form', rwViewJson: 'JSON', rwJson: 'JSON configuration', rwJsonHint: 'Advanced: edit the full configuration; the server validates it on save.',
        rwJsonInvalid: 'Invalid JSON', rwJsonArrayWarn: 'The top level must be a JSON object',
        rwScope: 'Scope', rwScopeHint: 'Only upstream 200 GET responses are rewritten; compressed, ranged, binary and responses over 1MB are skipped.',
        rwSaved: 'Response rewrite saved', rwCleared: 'Response rewrite cleared', rwClear: 'Clear rewrite', rwClearConfirm: 'Remove every response rewrite rule on this binding?',
        rwUnsafeHint: 'Rewriting responses places the gateway between content and browsers: use trusted content only, and never rewrite security headers or inject scripts.',
      ...
      menuEditor: {
        ...
        dynamicGroupHint: 'The "Local services" group is generated by port discovery and is not edited here.',
        ...
        builtinUndeletable: 'Built-in menu entries cannot be deleted',
        labelPlaceholder: 'e.g. Monitoring', urlPlaceholder: 'e.g. /_authz/apps/users.html or https://example.com',
        iconPlaceholder: 'MDI icon name, e.g. mdi-monitor', iconHint: 'Leave blank for the default icon', iconSearchPlaceholder: 'Search icons (e.g. chart, server, cloud...)', iconNoMatch: 'No matching icons, try another keyword',
