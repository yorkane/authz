  ...
  const messages = {
    'zh-CN': {
      ...
      menuEditor: {
        ...
        dynamicGroupHint: '“本地服务”分组自动汇集端口探测发现的本机服务（未绑定域名），条目固定不可移除。',
        domainsGroupHint: '“域名服务”分组自动汇集已绑定域名的应用（含其直连端口入口），条目固定不可移除。',
    ...
    'en-US': {
      ...
      menuEditor: {
        ...
        dynamicGroupHint: 'The "Local services" group auto-collects discovered local services (no domain binding); its items are fixed.',
        domainsGroupHint: 'The "Domain services" group auto-collects applications with domain bindings (plus their direct port entries); its items are fixed.',
