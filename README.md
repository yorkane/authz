...
| 入口 | 上游 | 说明 |
|------|------|------|
| `6080` (http) | `http(s)://<target_ip>:<port>` | 默认 308 到 HTTPS（`AUTHZ_HTTP_MODE=redirect`）；`serve` 模式才代理本机或其他 IP 的 HTTP/HTTPS 服务 |
...
- 预览：图片缩略图与放大、视频（点击后弹窗播放，走 nginx 原生 Range 分段）、音频（播放器占预览区 80% 宽，保证进度条与时间不被原生控件省略）、HTML 页面（经 `/_authz/files/` 静态入口并附加 `Content-Security-Policy: sandbox` 隔离渲染）、常见文本/JSON 内联预览；
- 图标按文件类型着色：目录琥珀、HTML 橙、文本/脚本绿、JSON 黄、图片暗红、视频蓝、音频青；
- 键盘操作：↑↓←→ 移动焦点、Enter 进入目录或打开预览、Backspace 返回上级、Delete 删除选中项（弹确认框）、PageUp/PageDown 翻页、g/l/d 切换视图、f 聚焦搜索、Esc 关闭预览；
- 预览区手势：图片点开即关（light-box 习惯）；视频/音频点画面切播放/暂停，底部原生控件条（进度条、音量）仍留给控件本身；触屏在图片/视频画面上左右滑动切上一/下一个文件（音频进度条与视频控件条内的横向拖拽留给原生 seek）；
- 移动端/触屏：单击条目即打开预览；预览头部在窄屏与触屏下自动收缩为纯图标（隐藏「新窗口打开」「下载」文字与计数），关闭按钮改为实心高亮并始终留在头部右侧，因此没有物理键盘（按不到 Esc）也能单手关闭；桌面仍显示带文字的按钮。
...
docker run -d --name gw --network host \
  ...
  ghcr.io/yorkane/authz:latest

# 浏览器打开 http://<host>:6080/_authz/apps/ 登录
...
- 所有管理 API 位于 `/_authz/api/`，响应使用 `application/json`；登录、OAuth 与 API 统一收敛在 `/_authz/` 单一 router 入口下。
...
- `viewer` 已退役并由 `guest` 接管：存量数据由迁移自动改写为 guest；现在提交 `viewer`
 ...
 - `guest` 是匿名用户角色，默认能力面只有两条：只读探针 `/_authz/guest`（**明文完整**回显当次
   ...
   代理范围与其他角色相同，用策略（`role:guest` 主体）配置。
...
- `admin/index.html` 是 SSI 管理壳，菜单由 `/_authz/apps/menu.html` 直接组装，不使用菜单 iframe；右侧应用仍使用
  iframe 加载 `users.html` 与 `authorization.html`（与壳同级），并隐藏 iframe 边框。
...
`guest`；管理员可在本机覆盖角色，并与其他远程身份共用 Casbin 授权。
...
| Registry | 地址 |
|----------|------|
| GitHub Container Registry | `ghcr.io/yorkane/authz` |
| Docker Hub | `yorkane/authz` |
...
# 或
docker pull yorkane/authz:latest
```
...
# 指定自定义镜像
bash test/run_tests.sh ghcr.io/yorkane/authz:1.31.1.1
```
